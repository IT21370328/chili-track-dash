...
const Purchasing = () => {
  ...
  const exportToCSV = async () => {
    ...
    try {
      await logAction(currentUser, "Export Purchases", `Exported ${filteredPurchases.length} purchase records`);
      toast({ title: "✅ Exported", description: "Purchases exported successfully." });
    } catch (error) {
      console.error("Failed to log export action:", error);
      toast({
        title: "Warning",
        description: "Purchases exported, but failed to log action.",
        variant: "default",
      });
    }
  };
      ...
      if (editingId) {
        ...
        try {
          await logAction(
            currentUser,
            "Update Purchase",
            `Updated purchase ID: ${editingId} (${quantity} kg, ${color}, ${supplier})`
          );
        } catch (error) {
          console.error("Failed to log update purchase action:", error);
        }
        ...
        try {
          await logAction(
            currentUser,
            "Add Purchase",
            `Added ${quantity} kg (${color}) from ${supplier}`
          );
        } catch (error) {
          console.error("Failed to log add purchase action:", error);
        }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      try {
        await logAction(currentUser, "Delete Purchase", `Deleted purchase ID: ${id}`);
      } catch (error) {
        console.error("Failed to log delete purchase action:", error);
      }
  ...
  const handleMarkAsPaid = async (id: number) => {
    try {
      ...
      try {
        await logAction(currentUser, "Mark Purchase as Paid", `Marked purchase ID: ${id} as paid (cash)`);
      } catch (error) {
        console.error("Failed to log mark as paid action:", error);
      }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{editingId ? "Edit Purchase" : "Add Purchase"}</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Button
              type="submit"
              className="col-span-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center gap-2"
            >
              <Plus className="w-4 h-4" /> {editingId ? "Update Purchase" : "Record Purchase"}
            </Button>
          </form>
        </CardContent>
      </Card>
