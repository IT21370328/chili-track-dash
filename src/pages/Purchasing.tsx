...
const Purchasing = () => {
  ...
  <div className="max-w-7xl mx-auto space-y-8">
    ...
    {/* Summary Cards */}
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 sm:gap-6 overflow-x-auto">
      {[
        ...
        <Card key={idx} className="bg-white/90 shadow-lg rounded-2xl flex flex-col p-4">
    ...
    <Card className="bg-white/90 shadow-lg rounded-2xl p-4 sm:p-6">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <select value={formData.paymentMethod} onChange={e => setFormData({...formData, paymentMethod: e.target.value})} required className="border rounded-lg p-2">
            <option value="">Select payment</option>
            <option value="cash">Cash</option>
            <option value="credit">Credit</option>
          </select>
          <select value={formData.color} onChange={e => setFormData({...formData, color: e.target.value})} required className="border rounded-lg p-2">
    ...
    <Card className="bg-white/90 shadow-lg rounded-2xl p-4 sm:p-6">
      <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
          ...
          {(dateFrom || dateTo) && <Button variant="outline" size="sm" onClick={resetDateFilter} className="mt-2 sm:mt-0">Reset</Button>}
        </div>
      </CardHeader>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2 flex-wrap">
                    ...
                    {p.paymentMethod === "credit" && (
                      <Button variant="secondary" size="sm" onClick={() => handleMarkAsPaid(p.id)}>Mark as Paid</Button>
                    )}
                  </div>
                </TableCell>
              </TableRow>
